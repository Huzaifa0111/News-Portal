...
// Code for Forever deletion
    if ($_GET['action'] == 'perdel' && $_GET['scid']) {
        ...
        $qlist = mysqli_query($con, "select id from tblposts where SubCategoryId='$id'");
        if (mysqli_num_rows($qlist) > 0) {
            while($curitem = mysqli_fetch_array($qlist)) {
                $query = mysqli_query($con, "delete from  tblcomments  where postId='$curitem[0]'");
            }
        }
        $query = mysqli_query($con, "delete from  tblposts  where SubCategoryId='$id'");
